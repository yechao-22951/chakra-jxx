...
PropId make_prop_id(const wchar_t *name) {
    ...
    if (err)
        jxx_throw_error(ERR_MSG(JxxOutOfMemory));
    return out;
}
...
class object_accessor_ : public js_value_t {
  ...
  public:
    ...
    class Property {
      ...
      public:
        Property() {}
        Property(js_value_t target, PropId prop)
    ...
    Property operator[](PropId prop_id) { return Property(*this, prop_id); }

    template <typename T> Property operator[](T &&x) {
...
JXXAPI JsValueRef JxxCreateExObject(JsValueRef prototype,
                                    IJxxNativeObject *native) {
    JsValueRef out = nullptr;
    auto err = JsCreateExternalObjectWithPrototype(
        native, JxxNativeObjectRelease, prototype, &out);
    if (err)
        return nullptr;
    if (native)
        native->AddRef();
