...
#define JXX_EXPORT(K, name)                                                    \
    static inline const __DECLARED_POINT__ __jxx__##name = K::ADD_EXPORT_ITEM( \
...
template <class CXX, typename... PARENTS> class JXX_CLASS : public PARENTS... {
  protected:
    static inline std::vector<JXX_EXPORT> __EXPORTED_ITEMS__;

  public:
    enum { __VISIBILITY__ = true };
    enum { __PARENTS_COUNT__ = sizeof...(PARENTS) };
    static inline const JXX_CLASS_ID __PARENTS__[] = {
        JXX_DEFINITION_OF_<PARENTS>..., nullptr};
    static inline JXX_BOOL __PARENTS_VISABLE__[] = {
        (PARENTS::__VISIBILITY__)..., false};
    static JXX_EXPORTS __EXPORTES__() {
        return JXX_EXPORTS {
            __EXPORTED_ITEMS__.size(), __EXPORTED_ITEMS__.empty() ? nullptr,
                __EXPORTED_ITEMS__.data();
        };
    }
    static int __MIXIN__(void *obj) {
        ...
        for (auto &exp : __EXPORTED_ITEMS__) {
        }
        return 0;
    }

  protected:
    static __DECLARED_POINT__ ADD_EXPORT_ITEM(JXX_NAME Name, JXX_COUNT argc,
                                              JXX_CALLEE JxxFunc) {
        __EXPORTED_ITEMS__.push_back({Name, JxxFunc, argc});
  ...
  protected:
    ...
    void *_query_class(JXX_CLASS_ID clsid) {
...
class JxxPipeline : public JXX_CLASS<JxxPipeline, JxxIo> {
  public:
    virtual _the<object> aw_read(_the<number> size, more_) = 0;
    virtual _the<number> aw_write(_the<object | buffer | string> buffer,
                                  more_) = 0;
    virtual _the<number> aw_close(_the<buffer | string> buffer, more_) = 0;
    JXX_EXPORT(JxxAwaitableIo, aw_read);
    JXX_EXPORT(JxxAwaitableIo, aw_write);
};
